...
function compareCharCodeAtAndCodePointAt(string, index) {
  ...
  return `charCodeAt: ${charCodeAtResult}, codePointAtResult:${codePointAtResult}, charAt:${charAtResult}, at:${atResult}`;
}
...
function padString(original, length, padWith, side) {
  ...
  return side === "start"
    ? original.padStart(length, padWith)
    : side === "end"
    ? original.padEnd(length, padWith)
    : "Error: side should be either 'start' or 'end'";
}
...
function trimString(original, side) {
  ...
  return side === "start"
    ...
    : side === "both"
    ? original.trim()
    : "Error: side should be either 'start', 'end' or 'both'";
}
